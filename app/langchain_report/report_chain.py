...
@tool
def search_treatment_reflections(patient_id: int, top_k: int = 3) -> str:
    """检索该患者历史上被医生正面反馈验证过的诊疗经验摘要（来自反思记忆库）。
    当需要参考"过去被证实有效的处理思路/用药经验"，而不仅仅是"影像相似的病例"时，调用这个工具。
    如果当前病例的处理方案已经很明确，不需要调用；如果想知道该患者过去是否有类似情况被验证过的经验，再调用。
    """
    try:
        results = query_top_reflections(patient_id, top_k)
    except Exception as e:
        return f"诊疗经验检索异常：{e}"
    if not results:
        return f"患者 {patient_id} 暂无已验证的诊疗经验记录。"
    lines = [f"- {r['summary']}（可信度 {r['rewardScore']}）" for r in results]
    return "检索到以下已验证诊疗经验：\n" + "\n".join(lines)


TOOLS = [search_similar_patients, search_treatment_reflections]
...
你有以下两个工具可以调用，是否调用、调用几次都由你自行判断：
1. search_similar_patients：检索历史上与当前患者CT特征相似的病例，用于影像层面的横向参考。
2. search_treatment_reflections：检索该患者过去被医生正面反馈验证过的诊疗经验，用于判断"以前类似情况是怎么处理、有没有效"。

如果当前影像和检验数据已经足够支撑明确结论，不需要调用任何工具；
如果存在不确定、需要历史病例或既往经验佐证，再按需调用，两个工具可以都用，也可以只用其中一个。最多调用2次。
...
当你不再需要调用工具、可以给出最终结论时，只输出以下JSON格式，不要输出其他任何内容（包括不要输出```标记）：
{
    ...
    "referenced_reflections": "如果调用过诊疗经验检索，说明参考了哪些经验、如何影响了你的判断；未调用则填'未参考'"
}
"""
...
def node_correlation_finalize(state: ReportState) -> dict:
    ...
    return {
        ...
        "chain_steps": [{
            ...
            "tools_called": list(set(state.get("correlation_tools_called", []))),   # ← 新增
        }],
    }
